...
export default function SunButton({sun, active} : {sun : number, active:boolean}){
  ...
  return(
    ...
    onPressOut={()=>[setActivation(!activation), setColor0('transparent')]}>
      {activation ? <SunFocusSvg w='20' h='20'/> : <SunSvg w='20' h='20'/>}
      <text.Body3 color={activation ? colors.white : colors.primary} style={{marginLeft:4}}>{cal.numName(sun)}</text.Body3>
    </button.SunButton>
  );
}