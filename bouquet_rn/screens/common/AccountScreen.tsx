    ...
    <area.Container>
      ...
      >
        ...
        >
          <elses.CircleImg
            diameter={120}
            source={{ uri: isMe ? user.profileImg : viewUser.profileImg }}
          />
          <text.Subtitle2B color={colors.black} style={{ marginVertical: 8 }}>
            {isMe ? user.name : viewUser.name}
          </text.Subtitle2B>
          <area.RowArea style={{ justifyContent: 'center' }}>
            <ProfileInfoText
              bold={cal.numName(totalFollowers).toString()}
              regular={i18n.t('팔로워')}
            ...
            <ProfileInfoText
              ...
              regular={i18n.t('캐릭터') + (i18n.locale === 'en' ? 's' : '')}
              color={colors.primary}
              center={1}
            />
          </area.RowArea>
        ...
        <text.Subtitle3 color={colors.black} style={{ marginBottom: 16 }}>
          {i18n.t('캐릭터')}
        </text.Subtitle3>

        <area.RowArea style={{ marginBottom: 12 }}>
          <text.Body2R color={colors.black}>{i18n.t('총')} </text.Body2R>
          <text.Body2B color={colors.black}>
            {isMe
              ? characterList.length.toString()
              : viewUser.characters.length.toString()}
          </text.Body2B>
        ...
        <FlatList
          ...
          renderItem={(obj) => {
            return (
              <View
                style={{ flex: 0.5, marginBottom: 13, marginHorizontal: 8 }}
              >
                {obj.item.name === '' ? (
                  <View />
                ) : (
                  <TouchableWithoutFeedback
                    onPress={() => setSelectId(obj.index)}
                  >
                    <ProfileChaItem
                      name={obj.item.name}
